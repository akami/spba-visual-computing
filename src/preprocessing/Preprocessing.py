...
class Preprocessing:
    ...
    def preprocess(self, img_file_path, in_the_wild=False):
        ...
        if in_the_wild:     # in-the-wild images need to be translated/aligned according to FFHQ data
            ...
            lm = self.get_landmark(dlib_img, self.predictor)

            # align face

            aligned_img = self.align_face(lm, img)
            img = aligned_img
    ...
    ############# COPY END #############
    """
    def align_face(self, lm, img, output_size=1024):
        """
    ...
    def get_landmark(self, img, predictor):
        """
